...
import { AdoptionPostListFilters } from "./components/adoption_post_list_filters";
          ...
          <div className="p-6">
            {posts.length > 0 ? (
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
                {posts.map((post) => (
                  <AdoptionPostCard key={post.id} post={post} linkPrefix="/adoption_posts" />
                ))}
              </div>
            ) : (
